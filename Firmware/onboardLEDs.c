...
void FlashI2CTx(int count){
	FlashLED1();
}

void FlashI2CRx(int count){
	FlashLED2();
}
...
void onboardLEDs_Init(void){
  ...
  GPIO_PORTN_AMSEL_R &= ~0x03;     								// disable analog functionality on PN0,PN1
	...
	GPIO_PORTF_DIR_R |= 0x13;        								// make PN4,0 out 
  GPIO_PORTF_AFSEL_R &= ~0x13;     								// disable alt funct on PF4,PF0, PF1
  GPIO_PORTF_DEN_R |= 0x13;        								// enable digital I/O on PF4,PF0, PF1
																									// configure PF4,PF0, PF1 as GPIO
  //GPIO_PORTN_PCTL_R = (GPIO_PORTN_PCTL_R&0xFFF0FFF0)+0x00000000;
  GPIO_PORTF_AMSEL_R &= ~0x013;     								// disable analog functionality on PF4,PF0, PF1
